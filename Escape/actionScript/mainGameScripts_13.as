...
qlw = 60
	...
	testClip.slideTo(solvedX*50+gridXOffset+100,solvedY*50+gridYOffset,1,"",.5)
		...
		//levelPoints = moves
			currentLevelhigh = Number(playerScoreArray[(level-1)])
			if (currentLevelhigh==undefined){
				
				currentLevelhigh=0
			}
			bestLevelScoreTextBox.text=Number(playerScoreArray[(level-1)])
			trace("levelBonus = "+levelBonus)
		if (levelBonus <0){
				...
				// add 500 points for completing the level
			//	levelBonus+=500
		//addPoints(	levelPoints)
		if (levelBonus>currentLevelhigh){
			
			//function addPoints(temp_Score:Number){

playerScoreArray[(level-1)]=levelBonus
	
bestLevelScoreTextBox.text=levelBonus
...
scoreTextBox.text = score
		...
		break
		case "setup level":
			currentLevelhigh = Number(layerScoreArray[(level-1)])
			bestLevelScoreTextBox.text=currentLevelhigh
			trace("currentLevelhigh = "+currentLevelhigh)
		levelBonus=1000
	bonusTextBox.text=levelBonus
	addUpScore()
	if (Number(playerScoreArray[(level-1)])==undefined){
		bestLevelScoreTextBox.text="0"
	}else{
bestLevelScoreTextBox.text=Number(playerScoreArray[(level-1)])
	}
		setupLevel();
