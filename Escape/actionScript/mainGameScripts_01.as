		...
		for (i=0; i<blockNameArray.length+1; i++) {
			...
			if (testClip != hitTestClip) {
				if (testClip.hitTest(hitTestClip)) {
					var collisionRect:Rectangle = CollisionDetection.checkForCollision(hitTestClip, testClip, 120);
					if (collisionRect) {
						//trace(collisionRect);
						trace(collisionRect.x);
						if (collisionRect.x<=testClip._x) {
							testClip._x = lastX;
							xfail =true
							
						}else
if (collisionRect.width+testClip._x>=testClip._x) {
							testClip._x = lastX;
								xfail =true
						...
						if (collisionRect.y<=testClip._y) {
							...
							if (collisionRect.width+testClip._y>=testClip._y) {
							testClip._y = lastY;
								yfail =true
						...
						//testClip._y = lastY;
						// user is trying to drag the block through another
						//	trace("bad drag")
						//	trace("lastX = "+lastX)
						snapBlocktoGrid()
					...
					testClip._x = currentXPosition;
					lastY=	currentYPosition
					testClip._y = currentYPosition;
				}
			}
		}
	}
